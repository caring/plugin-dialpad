...
const styles = theme => ({
  ...
  chipFocused: {
    backgroundColor: emphasize(
      theme.palette.type === 'light' ? theme.palette.grey[300] : theme.palette
      .grey[700],
      0.08,
    ),
  },
...
function Option(props) {
  return ( <
    MenuItem buttonRef = { props.innerRef } selected = { props.isFocused } component =
    'div'
    style = {
      {
...
export class ConferenceButton2 extends React.Component {
    ...
    pasteListener = (e) => {
      ...
      this.setState({
        screenMainLine: (typeof this.state.screenMainLine ===
          'undefined' ? paste : this.state.screenMainLine + paste)
      });
    }
    ...
    buttons = this.numbers.map((row, i) => {
            ...
            div className = { buttonRow } key = { i } > {
              row.map(function(item, i) {
                  return ( < Button size = 'large'
                    key = { item } onClick = {
                      e => originalThis.buttonPress(
                        item, originalThis.props.activeCall)
                    } > { item } <
                    /Button>);
                  })
              } <
          ...
          dialPad = () => {
            ...
            if (this.state.mode === 'dialpad') {
              ...
              return ( <
                ...
                NoSsr >
                <
                Select classes = { classes } styles = { selectStyles } options = { suggestions } components = { components } value = {
                  this
                  .state.single
                }
                onChange = {
                ...
                className = { iconButtons } component = 'div' >
                <
                Forward className = { forwardButton } onClick = {
                  e => this
          ...
          screen = () => {
            if (this.props.activeView === 'agent-desktop' || this.props.activeView ===
              'agentdesktop') { //Do not render the screen display if displaying on the main screen
              return <div / > ;
            } else {
              return ( < div className = { screen } >
                ...
                p className = { screenMainLine } > { this.state.screenMainLine } <
                /p> <
                Backspace className = { backspaceButton } onClick = {
                  e =>
                  this.backspace()
                }
                /> < /
            ...
            plusButton = () => {
              if (this.props.activeView === 'agent-desktop' || this.props.activeView ===
                ...
                return ( < div >
                  <
                  Button size = 'large'
                  key = 'placeHolder' > < /Button> <
                  Button size = 'large'
                  key = '+'
                  onClick = { e => this.buttonPress('+', this.props.activeCall) } >
                  + < /Button> <
                  Button size = 'large'
                  key = 'placeHolder2' > < /Button> < /
                  div > )
              }
            }

            keyPressListener(e) {
              if ((e.keyCode > 47 && e.keyCode < 58) || e.keyCode ===
                187) { //listen to 0-9 & +
            ...
            backspace() {
              this.setState({
                screenMainLine: this.state.screenMainLine
                  .substring(0, this.state.screenMainLine.length -
                    1)
              });
            }
            ...
            buttonPress(key, activeCall) {
              ...
              if (this.state.mode === 'dialpad') {
                ...
                if (key === '+') {
                  ...
                  this.setState({
                    screenMainLine: (typeof this.state.screenMainLine ===
                      'undefined' ? key : this.state.screenMainLine +
                      key)
                  });
                }
              }
            ...
            addConferenceParticipant() {
              ...
              const to = typeof(this.state.transferTo) === 'object' ?
                this.state.transferTo.value : this.state.screenMainLine;
              const from = typeof(this.state.transferTo) === 'object' ?
                this.props.workerName : this.props.from;

              fetch(
                  `https://${this.props.runtimeDomain}/add-conference-participant`, {
                    headers: {
            ...
            displayConferenceKeypad() {
              this.setState({
                mode: this.state.mode === 'conference' ?
                  'none' : 'conference'
              });
            }
            ...
            render() {
              return ( <
                ...
                className = { iconButtons } component = 'div' >
                <
                GroupAdd className = { conferenceButton } onClick = {
                  e =>
                  this.displayConferenceKeypad()
                }
                /> <
                GridOn className = { keypadButton } onClick = {
                  e =>
                  this.displayDialpad()
                }
                /> < /
